...
const wrongAnswerString = document.querySelector(
  ".playing-field__wrong-answer"
);
const expressionLabel = document.querySelector(".control__expression");
...
const wrongAnswerAnimation = () => {
  ...
  wrongAnswerString.addEventListener("transitionend", () =>
    wrongAnswerString.classList.remove("playing-field__wrong-answer--animation")
  );
...
const makeNewDrop = (indent, bonusDrop) => {
  ...
  operation.classList.add("drop__symbol");
...
const finishGame = (timer) => {
  ...
  equationsPerMinute =
    roundDuration >= 1
      ? totalEquationsSolved / roundDuration
      : totalEquationsSolved;

  saveStatistic();
  clearInterval(timer);

  document.location.href = "./end-game.html";
};
...
const checkAnswer = () => {
  ...
  const isCorrectAnswer =
    displayCalculator.textContent === String(firstDrop.result);
...
const refreshExpressionLabel = () => {
  const activeElement = document.querySelector(".playing-field .drop");
  if (activeElement) {
    const symbol = activeElement.querySelector(".drop__symbol").textContent;
    const firstNumber = activeElement.querySelector(".drop__numbers--first")
      .textContent;
    const secondNumber = activeElement.querySelector(".drop__numbers--second")
      .textContent;
    expressionLabel.textContent = `${firstNumber} ${symbol} ${secondNumber}`;
  }
};
...
const runEmulation = (finish) => {
  let emulationLimit = 10;
  const enterResult = (value) => {
    displayCalculator.textContent = value;
    setTimeout(() => {
      document.querySelector(".control__button--enter").click();
    }, 1000);
  };
  const failTry = () => {
    console.log("success");
  ...
  const getRightAnswer = () => {
    if (expressionLabel.textContent.length) {
      return Function(`return (${expressionLabel.textContent})`)();
    }
  };
  setInterval(() => {
    if (emulationLimit > 0) {
      if (expressionLabel.textContent.length) {
        emulationLimit--;
        Math.random() < 0.5 ? successTry() : failTry();
      }
    } else {
      levelSea = 300;
      finish();
